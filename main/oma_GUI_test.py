...
class OmaApp:
    ...
    def _post_process(self, params, ss, efdd, ssidat):
        ...
        self.save_cluster(params, ssi_save_folder, clusters, large_clusters, outlier=True)

        # mpe
        self.mpe_from_clusters(params, ssi_save_folder, large_clusters, ss, ssidat, efdd)
    ...
    def poles_to_df(self, ssidat_result):
        ...
        df_poles['col']     = coords[1] # model order
        df_poles['Fn']      = Fn
        df_poles['Phi']     = Phi
        df_poles['Xi']      = Xi
    ...
    def valid_compute_min_distances(self, df_poles):
        ...
        for order in arr_model_order:
            if order == 0:
                continue

            arr_pole_index = df_poles.query('col == @order')['row'].unique()
            ...
            for index in arr_pole_index:
                ...
                if len(other_poles) > 0:
                    df_min_distance.loc[order, index] = dist_i.min()
