...
class OmaApp:
    ...
    def initialize_widgets_of_params(self):
        ...
        self.param_etr3_0_2_1.insert(0, '100')
    ...
    def _mpe_from_efdd_and_ssidat(self, params, ss, efdd, ssidat):
        ...
        for med_or_mean in ['med', 'mean']:
            ...
            df_mp = self.modal_params_of_med_or_mean_values_of_xi_in_clusters_to_df(clusters, df_poles, med_or_mean, p['min_cluster_elements'])
            ...
            # create dataframe
            df_valid = self.cluster_and_phi_to_df(
                df_mp['ClusterNo.'].to_numpy(),
                ssidat.result,
                efdd.result,
                p['geo_path'],
                valid_poles_list,
    ...
    def hcluster_to_df(self, poles_list, clusters, min_cluster_elements):
        ...
        # drop small clusters whose number of elements is under min_cluster_elements
        ...
        df_large_clusters_info_with_freqs.columns = [' '] + [f"mode{i}" for i in range(1, len(df_large_clusters_info_with_freqs.columns))]
    ...
    def modal_params_of_med_or_mean_values_of_xi_in_clusters_to_df(self, clusters, df_poles, med_or_mean, min_cluster_elements):
        ...
        # extract clusters with a large number of cluster elements
        n, counts = np.unique(clusters, return_counts=True)
        num_of_large_cluster = n[counts >= min_cluster_elements]

        # indexes to extraxt modal parameters
        idx_mp = []
        for i in num_of_large_cluster:
            df = df_clusterd_poles[df_clusterd_poles['ClusterNo.'] == i].copy()
    ...
    def phi_to_df(self, cluster_num, ssidat_result, efdd_result, geo):
        Fn   = ssidat_result.Fn
        Phi  = ssidat_result.Phi
        Xi   = ssidat_result.Xi

        mpc_list, mpd_list, one_sub_mpd_list, mcf_list = self.MPC_MPD_MCF(Phi)
        ...
        df_phi.loc['1 - MPD']    = np.round(one_sub_mpd_list, 6)
    ...
    def cluster_and_phi_to_df(self, cluster_num, ssidat_result, efdd_result, geo, poles_list, clusters, min_cluster_elements):
        _, _, df_freq_info, _ = self.hcluster_to_df(poles_list, clusters, min_cluster_elements)
        df_phi = self.phi_to_df(cluster_num, ssidat_result, efdd_result, geo).T
        df_cluster_info = df_freq_info[['ClusterNo.', 'Elements', 'CV_freq', 'CV_damp']].reset_index(drop=True).copy()
        df_cluster_info['1-CV(freq)'] = 1 - df_cluster_info['CV_freq']
        df_cluster_info['1-CV(damp)'] = 1 - df_cluster_info['CV_damp']

        df_cluster_phi = pd.merge(df_cluster_info, df_phi, on='ClusterNo.')

        col1 = df_phi.columns.to_list()
        col2 = df_cluster_info.columns.to_list()
        col = col1[:3] + col2[1:] + col1[3:]
        df_cluster_phi = df_cluster_phi.reindex(columns=col)

        df_cluster_phi.index = [f"mode{i+1}" for i in range(len(df_cluster_phi))]

        return df_cluster_phi.T
    ...
    def params_to_txt(self, output_path_param, fs, br, ordmax, calc_unc, nb, hide_poles, q, Wn, order, btype, freqlim, percentile, params):
        p = params
