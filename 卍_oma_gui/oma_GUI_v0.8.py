"""else:
    ...
    の else: xerr = None を else: xerr = [] に変更
    """
...
class OmaApp:
    ...
    def run_efdd_and_ssidat(self):
        for key in self.set_up_dict.keys():
            if self.set_up_dict[key]['var_to_run'].get():
                ...
                self._mpe_from_efdd_and_ssidat(parameters, ss, efdd, ssidat)
    ...
    def _mpe_from_efdd_and_ssidat(self, params, ss, efdd, ssidat):
        ...
        _, df_count_of_large_cluster_elements, _, df_Hclus = self.hcluster_to_df(ssidat.result, valid_poles_list, clusters, p['min_cluster_elements'])
        ...
        # save the mpe result
        result = ss.algolithms['SSIdat'].result
        result.Fn      = df_mp['Fn'     ].to_numpy()
        result.Phi     = df_mp['Phi'    ].to_numpy().T
        ...
        print(f"ss.:{ss.algolithms['SSIdat'].result.Fn}")
        print(f"ssisat.:{ssidat.result.Fn}")
    ...
    def poles_to_df(self, ssidat_result):
        ...
        df_poles = pd.DataFrame(
            values,
            columns=['ax0', 'ax1', 'Fn', 'Phi', 'Xi', 'Fn_std', 'Phi_std', 'Xi_std']
        )
    ...
    def hcluster_to_df(self, poles_list, clusters, min_cluster_elements):
        ...
        df_freqs_in_large_clusters.columns = df_freqs_in_large_clusters['ClusterNo.']  # 他のdfと連結しやすくする

        df_large_clusters_info_T = df_large_clusters_info.T.reset_index()
        df_large_clusters_info_T.columns = df_large_clusters_info.iloc[0]
        df_large_clusters_info_T = df_large_clusters_info_T.drop(df_large_clusters_info_T.iloc[0])
        df_large_clusters_info_T = df_large_clusters_info_T(drop=True)
    ...
    def modal_params_of_med_values_in_clusters_to_df(self, clusters, df_poles, med_key, min_cluster_elements):
        df_clusterd_poles = pd.concat([
            pd.DataFrame([clusters.T], columns=['ClusterNo.']),
            df_poles['Xi']
        ...
        idx_mp = []
        df = df_clusterd_poles
        for i in no_of_large_cluster:
            cluster = df[df['ClusterNo.'] == i]
            l = len(df)
            if l % 2 == 0:
    ...
    def cluster_phi_and_FR_to_df(self, ssidat_result, percentile, geo, efdd_result, selected_freq, poles_list, clusters, min_cluster_elements):
        ...
        df_cluster_phi = self.cluster_and_phi_to_df(ssidat_result, percentile, geo, poles_list, clusters, min_cluster_elements)
